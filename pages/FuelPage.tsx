        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {packages.map((pkg, idx) => (
              <div 
                key={idx} 
                className={`p-10 rounded-[3rem] border border-zinc-900 flex flex-col h-full transition-all hover:-translate-y-4 ${pkg.color === 'primary' ? 'bg-primary text-black' : 'bg-zinc-950 text-white'}`}
              >
                <span className={`text-[10px] font-black uppercase tracking-widest mb-4 inline-block ${pkg.color === 'primary' ? 'text-black/60' : 'text-primary'}`}>
                  {pkg.price}
                </span>
                ...
                <ul className="space-y-6 flex-1">
                  {pkg.features.map((feat, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm font-bold uppercase tracking-tight">
                      <span className={`w-2 h-2 rounded-full ${pkg.color === 'primary' ? 'bg-black' : 'bg-primary'}`}></span>
                      {feat}
                    </li>
                  ))}
                </ul>
                <Link 
                  to="/contact" 
                  className={`mt-12 block text-center py-5 rounded-2xl font-black uppercase tracking-widest transition-all ${pkg.color === 'primary' ? 'bg-black text-white' : 'bg-primary text-black'}`}
                >
                  Fuel Up
                </Link>
              </div>
          ...
          <div className="mt-24 bg-zinc-900 p-12 md:p-20 rounded-[3rem] text-center">
            ...
            <Link to="/contact" className="inline-block px-12 py-5 border-2 border-primary text-primary hover:bg-primary hover:text-black rounded-full font-black uppercase tracking-widest transition-all">
